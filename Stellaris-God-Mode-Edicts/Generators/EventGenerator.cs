...
public readonly struct EventGenerator
{
    ...
    private static string Join(IEnumerable<EventGenerator> generators) => "namespace = godEdict_toggle\n" + string.Join("", generators);

    public static string GenerateFile(Edicts edicts)
    {
        var eventElements = new List<EventGenerator>();
        var currentId = 0;
        for (var i = 0; i < edicts.Length; i++) eventElements.Add(new EventGenerator(edicts[i].Name, currentId++, currentId++));
